...
rag_data = [{
        ...
        "context": "스킬 트레이너는 특화된 지식을 모델에 학습시키는 기능입니다. 특정 서비스에 필요한 여러 스킬을 구성하여 이를 모델에 학습시켜 모델의 성능을 높일 수 있습니다. 스킬 트레이너를 활용하면 사용자에게 최신 정보나 신뢰할 수 있는 정보를 제공할 수 있고, 많은 양의 데이터를 매번 학습하기 어려운 한계를 보완하여 효율적으로 모델의 지식을 업데이트할 수 있습니다."
            ...
            {
        "category": "스킬셋 관리",
        "context": "스킬셋은 특정 분야에 필요한 여러 스킬의 묶음입니다. 예를 들어 항공권 조회, 항공권 예약, 호텔 조회 스킬 등을 사용할 수 있는 여행 스킬셋, 상품 검색, 장바구니 관리 스킬 등을 사용할 수 있는 쇼핑 스킬셋 등으로 구성할 수 있습니다.    스킬셋 생성  스킬셋을 생성하는 방법은 다음과 같습니다.    네이버 클라우드 플랫폼 콘솔에서 Services > AI Services > CLOVA Studio 메뉴를 차례대로 클릭해 주십시오.  My Product 메뉴를 클릭한 후 [CLOVA Studio 바로가기] 버튼을 클릭해 주십시오.  CLOVA Studio에서 스킬 트레이너 메뉴를 클릭해 주십시오.  [새 스킬셋 생성하기] 버튼을 클릭해 주십시오.  동일한 그룹 내에 이미 생성된 스킬셋이 있는 경우에는 [스킬셋 생성] 버튼을 클릭해 주십시오.  스킬셋 생성 창이 나타나면 다음 정보를 입력해 주십시오.  서비스 분야: 생성할 스킬셋의 서비스 분야. 서비스 분야가 없는 경우, '기타' 선택  스킬셋: 스킬셋 이름. 영문자, 한글, 숫자, 공백을 허용하며 20자 이내로 입력  스킬셋 설명: 해당 스킬셋이 제공할 기능에 대한 설명. 1,000자 이내로 입력  답변 형식: 생성될 답변에 대한 형식. 1,000자 이내로 입력. 작성 예시는 스킬셋 답변을 참고해 주십시오.  User Information: 스킬셋을 대표하는 로고, 스킬 관련 이메일 주소, API 정책 주소를 입력 (선택 항목)  스킬셋 이름의 중복 여부를 확인하기 위해 [중복 확인] 버튼을 클릭해 주십시오.  [저장] 버튼을 클릭해 주십시오.  생성된 스킬셋 목록을 확인하려면 [취소] 버튼을 클릭해 주십시오.  스킬셋에 스킬을 추가하려면 [스킬 생성] 버튼을 클릭해 주십시오.  스킬셋 답변  스킬셋의 최종 답변에 대한 형식을 지정할 수 있습니다. 대표적으로 다음과 같은 답변 유형이 적용될 수 있고, 자연어 형태로 다양하게 작성할 수 있습니다. 시나리오 수집 과정에서 스킬셋 답변 형식과 최종 답변이 일치하는지 확인해 주십시오. 만약 일치하지 않는 경우 스킬셋 답변 보완을 참조하여 최종 답변을 보완해 주십시오.    답변 유형	작성 예시  JSON 형식	  답변은 json 형식으로 답변하세요.  최종 답은 {"result": [{"key1": ..., "key2": ...}, ...]}와 같은 JSON 형태로 답변해 주세요.  표 형식	  테이블 형식으로 답변을 보여주세요.  최종 답은 테이블 형식으로 답변해주세요. 이때 "No." 컬럼은 필수입니다.  번호 매기기	  답변을 넘버링해 주세요.  최종 답은 1, 2, 3의 형태가 아닌 4, 5, 6과 같이 숫자 4부터 시작한 형태로 답변해 주세요.  마크다운 형식	  제품명을 마크다운 형식으로 Bold 처리해 주세요.  최종 답은 마크다운의 코드블럭 형식으로 답변해 주세요.  특정 문구 고정	  답변의 시작은 항상 "기다려 주셔서 감사합니다."로 고정해 주세요.   답변의 끝맺음은 항상 "질문에 답변이 되었을까요?"로 하세요.  스킬셋 답변 보완  최종 답변이 설정한 답변 형식에 맞지 않게 생성되는 경우, 시나리오 수집 화면에서 최종 답변을 형식에 맞게 수정해야 합니다. 예를 들어 스킬셋의 답변 형식에 "최종 답변을 JSON 형식으로 반환해 주세요."라고 입력했지만 실제로 생성된 최종 답변이 JSON 형식이 아닌 경우, 답변 형식을 다음과 같이 JSON 형식에 맞게 수정해야 합니다. 이러한 과정으로 시나리오를 수집한 후 스킬셋 튜닝을 진행하면 최종 답변의 형식이 개선된 형태로 나올 수 있습니다.    스킬셋 답변을 보완하는 방법에 대한 예시는 다음과 같습니다.    답변 형식: "최종 답변을 JSON 형식으로 반환해 주세요."  반환된 최종 답변: "2023년 8월 9일 기준 모자 쇼핑 구매전환 이용자 수는 다음과 같습니다. 페이지 뷰 수는 1,003,120, 이용자 수는 1,222,334입니다."  수정할 답변 형식의 작성 예시:  {     "20230809":{        "purchase":[           {              "pv":"1,003,120",              "puruser":"1,222,334"           }        ]     }  }  JSON  참고  스킬셋 답변 형식이 스킬 답변 형식보다 우선 순위가 높습니다. 스킬 답변 형식과 중복되지 않도록 작성해 주십시오.    스킬셋 편집  생성한 스킬셋의 서비스 분야, 스킬셋 이름, 설명, 답변 형식, User Information을 수정할 수 있습니다.    스킬셋의 정보를 수정하는 방법은 다음과 같습니다.    네이버 클라우드 플랫폼 콘솔에서 Services > AI Services > CLOVA Studio 메뉴를 차례대로 클릭해 주십시오.  My Product 메뉴를 클릭한 후 [CLOVA Studio 바로가기] 버튼을 클릭해 주십시오.  CLOVA Studio에서 스킬 트레이너 메뉴를 클릭해 주십시오.  [스킬셋] 탭에서 생성된 스킬셋 목록을 확인해 주십시오.  스킬셋 목록의 오른쪽 끝에 있는 더보기i_clovastudio-more> [편집] 버튼을 클릭해 주십시오.  스킬셋 편집 창이 나타나면 스킬셋 정보를 수정해 주십시오.  스킬셋 정보를 수정한 후, [저장] 버튼을 클릭해 주십시오.  편집할 내용이 없으면 [취소] 버튼을 클릭해 주십시오."
},
            {
        "category": "스킬 관리",
        "context": "스킬은 사용자의 요구 사항에 정확하고 적절한 응답을 주기 위해 사용하는 도구입니다. 모델은 사용자의 질문과 스킬에 작성된 API를 분석하여 가장 적절한 답을 줄 수 있는 스킬을 선택한 후, 정확한 정보를 호출하여 사용자에게 제공할 수 있습니다. 예를 들어 동물 병원 조회 요청에 대응할 수 있는 '동물 병원 조회 API', 여행지 추천 요청에 대응할 수 있는 '추천 여행지 API'를 하나의 스킬로 구성할 수 있습니다    스킬 생성  스킬을 생성하는 방법은 다음과 같습니다.    네이버 클라우드 플랫폼 콘솔에서 Services > AI Services > CLOVA Studio 메뉴를 차례대로 클릭해 주십시오.  My Product 메뉴를 클릭한 후 [CLOVA Studio 바로가기] 버튼을 클릭해 주십시오.  CLOVA Studio에서 스킬 트레이너 메뉴를 클릭해 주십시오.  [스킬셋] 탭에서 스킬을 생성할 스킬셋의 [목록] 버튼을 클릭해 주십시오.  화면 오른쪽 상단의 [스킬 생성] 버튼을 클릭해 주십시오.  스킬 정보 화면이 나타나면 스킬 이름을 입력한 후 [중복 확인] 버튼을 클릭해 주십시오.  스킬 이름은 영문자, 한글, 숫자, 공백을 허용하며 20자 이내로 입력해 주십시오.  해당 스킬셋 내에서 스킬셋 이름이 중복되지 않도록 입력해 주십시오.  답변 형식에 스킬을 통해 생성될 답변 형식을 입력해 주십시오.  API Spec 영역에 API 스펙을 JSON 타입의 OAS 3.0 버전으로 작성해 주십시오.  API Spec을 작성하는 방법은 API Spec 작성을 참조해 주십시오.  API 스펙 작성이 완료되면 [검증하기] 버튼을 클릭해 주십시오.  검증 성공 시 [완료] 버튼으로 토글됩니다.  검증 실패 시 오류 메시지가 나타납니다. API 스펙을 다시 수정해 주십시오.  Manifest 영역에 정보를 입력해 주십시오.  Manifest를 작성하는 방법은 Manifest 작성을 참조해 주십시오.  필수 정보를 모두 입력한 후, [저장] 버튼을 클릭해 주십시오.  현재까지 작업한 내용을 저장하려면 [임시저장] 버튼을 클릭해 주십시오.  모든 영역이 입력되었더라도 [임시저장] 버튼을 클릭하는 경우, 작업 상태가 '작업 중'으로 저장됩니다.  스킬 생성 완료 창이 나타나면 [확인] 버튼을 클릭해 주십시오.  참고  처음 스킬이 생성되면 버전 v1이 생성됩니다. 해당 버전을 기본 모델로 호출하고 싶을 경우 스킬트레이너 > 작업 > 버전관리 메뉴에서 테스트앱을 생성한 후 서비스앱을 신청하고 연동해 주십시오. 자세한 내용은 스킬셋 버전 관리를 참조해 주십시오.    API Spec 작성  API Spec 항목에는 모델이 이해할 수 있는 API 스펙을 작성합니다. 스킬 트레이너에서는 JSON 형식만 지원합니다. 자세한 명세는 OpenAPI Specification v3.0.0을 참조해 주십시오.    Version  사용할 OpenAPI 버전 정보입니다. 3.0 버전만 지원하며, 다른 버전을 사용할 경우 오류가 발생할 수 있습니다. 필수 입력 값입니다.    작성 예시는 다음과 같습니다.    {      "openapi": "3.0.0"  }  JSON  Info  제공되는 API에 관한 정보입니다. 필수 입력 값입니다.    필드	설명	필수 여부  version	API 버전 정보	Y  title	API 이름	Y  작성 예시는 다음과 같습니다.    info:    version: API 버전    title: API 제목    description: API 설명    contact: API 제공자    license: API 라이선스 정보  YAML  Servers  API가 제공되는 대상 호스트 정보로 Path들의 baseURL입니다. 필수 입력 값입니다.    필드	설명	필수 여부  url	서버 URL	Y  작성 예시는 다음과 같습니다.    servers:    - url: https://sample.naver.com/v1  YAML  Paths  제공되는 API의 각 Path 정보입니다. 필수 입력 값입니다. 각 Path의 하위에는 Operation Object와 Parameter Object가 존재합니다.    필드	설명	필수 여부  Summary I Description	사용할 파라미터와 해당 Path를 통해 얻을 수 있는 정보를 구체적으로 작성  <예시> 여행지의 국내외 여부, 지역명, 방문하는 달, 여행자 수, 예상 경비를 통해서 여행 지역 및 도시 검색  Y  참고  현재 requests get, requests post 메소드만 지원합니다.  POST 메소드인 경우, 'nested datatype'은 아직 정식 지원하지 않으므로 파라미터를 분리해서 작성해 주십시오.  URL은 반드시 argument 방식으로 동작하는 것으로 가정하여 작성해 주십시오. 현재의 모델은 argument 방식만 지원합니다. (파라미터=파라미터의 값)  <예시 1> http://test.com/data?birthdata=20190302&address=서초구 (O)  <예시2 > http://test.com/data?20190302/서초구 (X)  작성 예시는 다음과 같습니다.    paths:    /test:      get:        description: get test data (해당 path가 어떤 상황에서 쓰이는지 구체적으로 작성)        operationId: getTest        responses:          '200':            description: test response            content:              application/json:                schema:                  type: array                  items:                    $ref: '#/components/schemas/Test'  YAML  Parameter Object  작업에 적용할 수 있는 매개 변수 목록입니다. 필수 입력 값입니다.    필드	설명	필수 여부  name	매개 변수의 이름. 대소문자 구별	Y  in	매개 변수의 위치.  사용 가능한 값: query  Y  description	해당 파라미터에 대한 구체적인 설명	N  required	매개 변수의 필수 여부 결정  사용 가능한 값: true, false  N  schema: type	매개변수 데이터 유형 설정  사용 가능한 값: string, integer, number, boolean, array  N  schema:format	데이터 타입별 포맷 설정  데이터 타입별 설정 가능한 포맷은 OpenAPI Specification v3.0.0 참조  N  참고  description에는 해당 파라미터가 어떤 의미를 갖고 있는지, 어떤 상황에서 사용되는 것인지, URL 생성 시 어떤 형식으로 들어가야 하는지 등을 구체적으로 작성합니다. 만약 파라미터가 가져야 하는 값이 한정되어 있다면 예시 값을 구체적으로 작성합니다.    male 또는 female 둘 중 하나의 값만 가져야 한다면 description에 구체적으로 작성합니다.  <예시> male 또는 female 중에 하나  강남으로 검색해도 강남구로 url에 들어가야 한다면 description에 구체적으로 작성합니다.  <예시> 강남구, 서초구  해당 파라미터가 한정된 장소를 검색하는 값이라면, 단순 검색어가 아닌 '레스토랑 또는 카페와 같은 특정 장소를 검색할 때 사용된다'라는 의미를 모델이 이해할 수 있도록 작성합니다.  <예시>  {     "parameters": [        {           "name": "place_category",           "in":"query",           "description": "장소의 대분류. restaurant, cafe, attraction, accommodation 중에 하나.",           "required": false,           "schema": {              "type": "string"           }        }     ]  }  JSON  작성 예시는 다음과 같습니다.    parameters:  - name: birthdate    in: query    description: 생일을 의미합니다. 입력 형식은 yyyy-mm-dd입니다.    required: true    schema:      type: array      style: simple      items:        type: string        format: date  YAML  Operation Object  API 호출 작업에 대한 정보입니다. 필수 입력 값입니다.    필드	설명	필수 여부  description	작업 동작에 대한 자세한 설명	N  operationId	작업 식별 시 사용되는 고유 문자열. Open API를 이용하는 다른 툴이나 라이브러리에서 operationId를 사용하여 식별하는 것도 가능	Y  responses	작업을 실행하여 반환되는 것이 가능한 응답 목록. 응답은 Response Object 형태로 제공	Y  작성 예시는 다음과 같습니다.    get:    description: get test data    operationId: getTest    responses:      '200':        description: teset response        content:          application/json:            schema:              type: array              items:                $ref: '#/components/schemas/Test'      default:        description: unexpected error        content:          application/json:            schema:              $ref: '#/components/schemas/Error'  YAML  Response Object  작업 응답에 대한 설명입니다. 필수 입력 값입니다.    필드	설명	필수 여부  description	응답에 대한 간단한 설명	Y  참고  200 응답은 필수 값이며, HTTP 상태 코드에 따라 기타 응답을 추가할 수 있습니다.    잘못된 파라미터에 대한 요청 시 422, 400 에러 반환  정의된 에러 코드가 없을 경우, 422 에러를 임의로 만들어서 반환  작성 예시는 다음과 같습니다.    responses:    '200':  	description: test response  	content:  	  application/json:  		schema:  		  items:  			$ref: '#/components/schemas/Test'    default:  	description: unexpected error  	content:  	  application/json:  		schema:  		  $ref: '#/components/schemas/Error'  YAML  Components  OAS의 다양한 부분에서 재사용 가능한 객체 집합입니다.    필드 이름	타입	설명  schemas	Map[string, Schema Object I Reference Object]	재사용 가능한 Schema Object의 객체  responses	Map[string, Response Object I Reference Object]	재사용 가능한 Response Object의 객체  parameters	Map[string, Parameter Object I Reference Object]	재사용 가능한 Parameter Object의 객체  examples	Map[string, Example Object I Reference Object]	재사용 가능한 Example Object의 객체  작성 예시는 다음과 같습니다.    components:    schemas:      Test:        allOf:          - $ref: '#/components/schemas/NewTest'          - type: object            required:              - id            properties:              id:                type: integer                format: int64                example: 100      NewTest:        type: object        required:          - name        properties:          name:            type: string          tag:            type: string            example: "test tag"  YAML  API Spec 확장 기능  API Spec에서 x-exclude-cot 객체를 사용하면 API 응답에서 해당 파라미터를 제외할 수 있습니다. Response Object에서 사용되고, Content-Type이 application/json 타입인 경우에만 적용 가능합니다. 응답에 사용되는 모든 매개변수를 정확하게 작성해야만 정상적으로 적용됩니다.    API 응답에서 address라는 매개 변수를 제외하도록 설계한 예시는 다음과 같습니다.    "responses": {      "200": {          "description": "test response",          "content": {              "application/json": {                  "schema": {                      "properties": {                          "name": {                              "type": "string"                          },                          "address": {                              "type": "object",                              "properties": {                                  "city" : {                                      "type": "string"                                  },                                  "state": {                                      "type": "string"                                  }                              },                              "x-exclude-cot": true                          },                          "contact": {                              "type": "string"                          }                      }                  }              }          }      }  }  JSON  참고  다음과 같은 경우에 활용되면 좋습니다.    API 응답이 길어서 토큰 수 초과 에러가 발생하는 경우  x-exclude-cot가 적용된 파라미터의 경우 시나리오 수집 과정 중 스킬 데이터 영역 2단계(API 호출 단계)의 관찰에서 제외됩니다. 따라서 실제 사용되지 않는 파라미터들에 대해 x-exclude-cot를 적용함으로써 API 응답을 줄일 수 있습니다.  학습 비용을 절약하고자 하는 경우  불필요한 파라미터에 대하여 x-exclude-cot를 적용하게 되면, 시나리오 수집 과정에서 토큰 수를 절약할 수 있습니다. 결과적으로 학습에 사용되는 토큰 수를 절약할 수 있습니다.  API Spec 설계 시 주의 사항  API Spec 설계 시 주의해야 할 사항을 설명합니다.    필수 파라미터 누락 시 알림    API 호출 시 필수 파라미터가 누락된 경우, 다음의 예시와 같이 상태 코드 200과 누락된 파라미터를 알려주는 상태 메세지를 반환하도록 설계해 주십시오.    {      "detail": "{'status': 200, 'message: 'Required parameter theme, min_grade is missing.'}"  }  JSON  참고  단, HCX-002 엔진을 사용하는 경우에는 필수 파라미터가 누락되었을 때 다음과 같이 400 에러를 반환하도록 설계해 주십시오.    {      "detail": "{'status': 400, 'error': 'Bad Request', 'message': 'Required parameter theme, min_grade is missing.'}"  }  JSON  호출 파라미터 최소화    예를 들어 비행기 예약 API를 호출할 때 API로 얻을 수 있는 결과가 '출발지', '도착지', '항공사', '출발날짜', '도착날짜', '금액'이 기재된 다수의 데이터라고 가정합니다. 만약 API의 호출 파라미터를 '도착지'만 등록한다면 '김포 공항에서 출발해서 제주 공항에 도착하는 7월 21일에 출발하는 30만원 이하의 비행기 알려줘'라고 요청하더라도 실제로 호출 가능한 파라미터는 '제주'이기 때문에 API는 '제주'가 도착지인 모든 경우의 수의 값을 모두 반환합니다. 이렇게 될 경우 원하는 결과를 얻기 어렵습니다.  API로 얻을 수 있는 결과가 '출발지', '도착지', '항공사', '출발날짜', '도착날짜', '금액'이라면 호출 가능한 파라미터에 '출발지', '도착지', '항공사', '출발날짜', '도착날짜', '금액 이상 검색', '금액 이하 검색'과 같이 호출 가능한 파라미터를 상세하게 배치하는 것이 좋습니다. 각 파라미터는 필수 값일 수도 있고, 필수 값이 아닐 수 있습니다. '김포 공항에서 출발해서 제주 공항에 도착하는 7월 21일에 출발하는 30만원 이하의 비행기 알려줘' 라고 요청한다면 API는 실제로 해당 조건을 만족하는 결과만 반환하므로 원하는 답변을 정확하고 빠르게 얻을 수 있습니다.    파라미터의 중복 호출    한 번의 호출에 동일한 파라미터를 여러 개 호출하도록 설계할 경우, API가 원활하게 동작하지 않으므로 충분히 많은 데이터를 만들어야 합니다.  동일한 파라미터를 복수로 사용하는 경우에 대한 예시는 다음과 같습니다.    http://goodfood.or.kr/openApi/restful/food?drink=coke&drink=milk&menu=hamburger  Plain text  API Spec 작성 예시  API Spec 작성 예시는 다음과 같습니다.    Requests_get을 사용하는 예시  {      "openapi": "3.0.0",      "info": {          "version": "v0",          "title": "국내 항공기 운행 스케줄 검색"      },      "servers": [          {              "url": "http://test.airport.co.kr/service/rest/TestFlightScheduleList"          }      ],      "tags": [          {              "name": "open-ai-product-endpoint",              "description": "Open AI Product Endpoint. Query for products."          }      ],      "paths": {          "/getDflightScheduleList": {              "get": {                  "tags": [                      "open-ai-product-endpoint"                  ],                  "summary": "국내 항공기 운행 스케줄을 검색합니다.",                  "operationId": "productsUsingGET",          "parameters" : [  {            "name" : "serviceKey",            "in" : "query",            "description" : "serviceKey는 반드시 2FMD를 쓰세요",            "required" : true,            "schema" : {              "type" : "string",              "default" : "1"            }},            {            "name" : "schDate",            "in" : "query",            "description" : "검색 일자. 반드시 8자리의 숫자로 입력하세요. 예를 들어 2022년 4월 1일이면 20220401",            "required" : false,            "schema" : {              "type" : "string"            }          }, {            "name" : "schDeptCityCode",            "in" : "query",            "description" : "도착 도시 코드. 예를 들어 서울을 나타내는 GMP를, 도착도시코드는 제주도를 나타내는 CJU, 부산은 PSU",            "required" : false,            "schema" : {              "type" : "string",              "default" : "10"            }          }, {            "name" : "schArrvCityCode",            "in" : "query",            "description" : "출발 도시 코드. 예를 들어 서울을 나타내는 GMP를, 도착도시코드는 제주도를 나타내는 CJU, 부산은 PSU",            "required" : false,            "schema" : {              "type" : "string",              "default" : "10"            }          }                  ],                  "responses": {                      "200": {                          "description": "Products found",                          "content": {                              "application/json": {                                  "schema": {                                      "$ref": "#/components/schemas/Rss"                                  }                              }                          }                      },                      "503": {                          "description": "one or more services are unavailable"                      }                  },                  "deprecated": false              }          }      },      "components": {          "schemas": {              "Rss": {                  "type": "object",                  "properties": {                      "airlineKorean": {                          "type": "string",                          "description": "항공사(국문)"                      },                      "arrivalcity": {                        "type": "string",                        "description": "도착 공항"                      },                      "domesticArrivalTime": {                        "type": "integer",                        "description": "도착 시간"                      },                      "domesticEddate": {                        "type": "string",                        "format": "date-time",                        "description": ""                      },                      "domesticFri": {                        "type": "string",                        "description": "금요일"                      },                      "domesticMon": {                        "type": "string",                        "description": "월요일"                      },                      "domesticNum": {                        "type": "string",                        "description": "항공편명"                      },                      "domesticSat": {                        "type": "string",                        "description": "토요일"                      },                      "domesticStartTime": {                        "type": "integer",                        "description": "출발 시간"                      },                      "domesticStdate": {                        "type": "string",                        "format": "date-time",                        "description": ""                      },                      "domesticSun": {                        "type": "string",                        "description": "일요일"                      },                      "domesticThu": {                        "type": "string",                        "description": "목요일"                      },                      "domesticTue": {                        "type": "string",                        "description": "화요일"                      },                      "domesticWed": {                        "type": "string",                        "description": "수요일"                      },                      "startcity": {                        "type": "string",                        "description": "출발 공항"                      },                      "numOfRows": {                        "type": "integer",                        "description": "열 숫자"                      },                      "pageNo": {                        "type": "integer",                        "description": "페이지 번호"                      },                      "totalCount": {                        "type": "integer",                        "description": "데이터 총계"                      }                  }              }          }      }  }  JSON  Requests_post를 사용하는 예시  {    "openapi": "3.0.0",    "info": {      "title": "Free API Documentation",      "version": "1.0.0"    },    "servers": [      {        "url": "https://test.bakery.com"      }    ],    "paths": {      "/bakery_products": {        "post": {          "summary": "베이커리 제품 정보를 등록하는 API입니다.",          "description": "베이커리 제품 정보를 등록하는 API입니다.",          "operationId": "filterBakeryProducts",          "requestBody": {            "required": true,            "content": {              "application/json": {                "schema": {                  "type": "object",                  "properties": {                    "name": {                      "type": "string",                      "description": "제품 영문명"                    },                    "max_price": {                      "type": "integer",                      "description": "최대 가격 (원)"                    },                    "ingredients": {                      "type": "string",                      "description": "재료의 영문명"                    },                    "expiration_date": {                      "type": "string",                      "description": "유통 기한. 예시) 20220701"                    },                    "allergens": {                      "type": "string",                      "description": "알레르기 성분의 영문명"                    }                  },                  "required": [                    "max_price"                  ]                }              }            }          },          "responses": {            "200": {              "description": "성공적인 응답",              "content": {                "application/json": {                  "schema": {                    "type": "array",                    "items": {                      "type": "object",                      "properties": {                        "id": {                          "type": "integer",                          "description": "제품 ID"                        },                        "name": {                          "type": "string",                          "description": "제품 이름"                        },                        "price": {                          "type": "integer",                          "description": "가격 (원)"                        },                        "ingredients": {                          "type": "array",                          "items": {                            "type": "string"                          },                          "description": "재료 목록"                        },                        "expiration_date": {                          "type": "string",                          "format": "date",                          "description": "유통 기한"                        },                        "calories": {                          "type": "integer",                          "description": "칼로리"                        },                        "allergens": {                          "type": "array",                          "items": {                            "type": "string"                          },                          "description": "알레르기 성분 목록"                        },                        "additional_options": {                          "type": "array",                          "items": {                            "type": "string"                          },                          "description": "추가 옵션 목록"                        }                      }                    }                  }                }              }            }          }        }      }    }  }  JSON  Manifest 작성  Manifest는 해당 스킬을 통해 호출할 수 있는 API의 이름, 목적, 사용 방법 등을 입력하는 영역입니다. Manifest의 입력 영역인 schema version, Name for model, Description for human, Description for model 각 필드에 대해 설명합니다.    Schema version    스키마 버전을 입력합니다.    Name for model    API 이름을 작성합니다. 다른 API와 이름이 중복되지 않아야 하며 API의 성격을 내포하여 간결하게 작성해야 합니다.  영문자, 숫자를 허용하며 대문자로 시작하는 문자열을 20자 이내로 입력해 주십시오. 한글, 특수 문자, 공백을 허용하지 않습니다.  두 개 이상의 단어를 조합할 경우, 각 단어의 첫 글자은 대문자로 작성해 주십시오. 예를 들어, 'animal'과 'hospital'이라는 두 개의 단어를 조합하여 API 이름을 만들 경우 'AnimalHospital'라고 작성합니다.  스킬 이름에 'API'라는 단어는 사용하지 마십시오.  Description for human    API의 목적과 용도를 작성합니다. 모든 API의 역할이 명확하게 작성되어야 합니다. 사용자의 질문을 바탕으로 모델이 여러 API 중에서 필요한 API를 선택하는 기준이 됩니다. 예를 들어 동물병원 검색, 애견카페 검색, 애견용품 검색 API가 존재할 때 사용자의 질문이 '동물병원 위치 알려줘'일 경우, 모델은 각 API의 description_for_human을 읽고 동물병원 검색 API를 선택합니다.  작성 예시는 다음과 같습니다.    울산광역시에 있는 동물 병원을 찾을 때 사용합니다. 병원 이름으로 조회하여 동물 병원의 주소와 연락처 정보를 제공합니다.  Plain text  Description for model    API의 주요 사용 방법을 작성합니다. 모델이 사용자의 질문에 답변할 때 사용할 API를 결정할 때 사용자의 질문과 ‘decription_for_model’을 모두 참고하여 여러 개의 API 중 적절한 API를 선택합니다. 또한 API 응답으로 받을 수 있는 정보에 대해서도 작성해야 합니다. 각 API를 호출했을 때 어떤 정보를 받을 수 있는지에 대해 범위를 상세하게 작성해야 모델이 API를 적절하게 선택하여 사용할 수 있습니다. 만약 Path가 여러 개인 경우에는 Path별 역할에 대해 자세히 작성해야 합니다. Path별로 검색되어야 할 키워드를 작성해야 모델이 해당 API를 적절하게 사용할 수 있습니다.    전통 시장의 정보와 주차장 시설 정보를 조회하는 API입니다. 이 API를 호출하면 응답으로 장소명, 주소, 운영 시간 정보가 제공됩니다. 서울시 전통 시장의 정보를 알고 싶을 때에는 /traditional_market을 사용하세요. /traditional_market은 시장 이름, 영업일, 휴무일, 주소 키워드, 영업 시작 시간, 영업 종료 시간을 검색어로 입력할 때 동작합니다. 주차장 시설 정보를 얻고 싶을 때는 /parking_lot을 사용하세요. /parking_lot은 지역구분_구, 지역구분_동, 주차장 명, 주차장 종류, 운영시간, 최소 가격을 검색어로 입력할 때 동작합니다. 전송할 쿼리에는 관사, 전치사, 결정자와 같은 중지어가 포함되어서는 안 됩니다. 링크는 항상 반환되며 사용자에게 표시되어야 합니다.  울산광역시 동물 병원 현황 조회 API입니다. 이 API를 호출하면 응답으로 동물 병원 이름, 위치, 전화번호, 운영 시간 정보가 제공됩니다. 울산시에 있는 동물 병원을 검색할 때는 /getPetHospitalList를 사용하세요. /getPetHospitalList는 판매점 이름을 검색어로 입력할 때 동작합니다. 전송할 쿼리에는 관사, 전치사, 결정자와 같은 중지어가 포함되어서는 안 됩니다. 링크는 항상 반환되며 사용자에게 표시되어야 합니다.  스킬 편집  스킬 정보를 수정하는 방법은 다음과 같습니다.    네이버 클라우드 플랫폼 콘솔에서 Services > AI Services > CLOVA Studio 메뉴를 차례대로 클릭해 주십시오.  My Product 메뉴를 클릭한 후 [CLOVA Studio 바로가기] 버튼을 클릭해 주십시오.  CLOVA Studio에서 스킬 트레이너 메뉴를 클릭해 주십시오.  [스킬셋] 탭에서 정보를 수정할 스킬셋의 [목록] 버튼을 클릭해 주십시오.  스킬 목록의 오른쪽에 있는 [편집] 버튼을 클릭해 주십시오.  편집 창이 나타나면 스킬 정보를 수정한 후, [저장] 버튼을 클릭해 주십시오.  수정한 내용이 있거나 필수 항목이 모두 작성되어야 [저장] 버튼이 활성화됩니다.  편집할 내용이 없으면 [취소] 버튼을 클릭해 주십시오.  스킬 삭제  스킬을 삭제하는 방법은 다음과 같습니다.    네이버 클라우드 플랫폼 콘솔에서 Services > AI Services > CLOVA Studio 메뉴를 차례대로 클릭해 주십시오.  My Product 메뉴를 클릭한 후 [CLOVA Studio 바로가기] 버튼을 클릭해 주십시오.  CLOVA Studio에서 스킬 트레이너 메뉴를 클릭해 주십시오.  [스킬셋] 탭에서 스킬셋의 [목록] 버튼을 클릭해 주십시오.  스킬 목록이 나타나면 삭제할 스킬의 오른쪽 끝에 있는 더보기i_clovastudio-more.png > [삭제] 버튼을 클릭해 주십시오.  삭제 팝업 창이 나타나면 [삭제] 버튼을 클릭해 주십시오.  참고  본인이 생성한 스킬만 삭제할 수 있습니다."
},
            {
        "category": "시나리오 관리",
        "context": "시나리오는 사용자의 질문에 적절한 답을 주기 위해 모델이 생각하고 판단하는 과정입니다. 시나리오 수집 및 데이터 검토를 통해 모델의 사고 과정을 수정할 수 있습니다.    시나리오 화면  NCP 시나리오1.png    항목	설명  ① 시나리오 수집 영역	시나리오를 수집하기 위한 데이터를 설정하는 영역  스킬셋: 시나리오를 수집할 스킬셋 선택  포함된 스킬: 선택한 스킬셋에 포함된 스킬 목록이 표시됨. 작업 상태가 '작업 완료'인 스킬만 활용 가능  Engine 및 버전: 사용할 언어 모델이나 버전을 선택. 기본 언어 모델이 기본 값으로 선택되어 있음  호출옵션(선택): 유저쿼리 호출 시 고정 값 입력  유저쿼리: 사용자 쿼리를 입력하는 필드. 스킬셋을 선택해야 해당 필드가 활성화됨  [실행] 버튼: 시나리오 수집을 실행하는 버튼  ② 플래닝 데이터 영역	진행 중인 모델 사고 과정의 전체 데이터가 노출되는 영역  생각: 모델의 생각  액션: 현재 스텝을 진행하기 위해 필요한 액션 종류  액션 입력: 액션 호출에 필요한 입력 값  결과: 액션을 수행한 결과  ③ 스킬 데이터 영역	스킬을 호출한 데이터가 노출되는 영역  생각: 모델의 생각  액션: 현재 스텝을 진행하기 위해 필요한 액션 종류  액션 입력: 액션 호출에 필요한 입력 값  관찰: 모델의 판단에 따라 사용해야 하는 API 스펙 및 API 호출 결과  ④ 기능 버튼	  [불러오기] 버튼: 이전에 작업하던 시나리오를 불러오는 버튼  [임시저장] 버튼: 현재까지 작업한 내용을 저장하는 버튼  [작업 완료] 버튼: 현재까지 작업한 내역을 완료하는 버튼. 최종 답변 데이터가 생성된 경우에만 활성화  참고  Step 1의 결과는 스킬 호출에 대한 결과가 표시되며, 나머지 스텝에서는 전체 모델 생성에 대한 결과가 표시됩니다.  최종 답변이 생성되었더라도 [임시저장] 버튼을 클릭하는 경우, 작업 상태가 '작업 중'으로 변경됩니다.  시나리오 수집  시나리오를 수집하는 방법은 다음과 같습니다.    네이버 클라우드 플랫폼 콘솔에서 Services > AI Services > CLOVA Studio 메뉴를 차례대로 클릭해 주십시오.  My Product 메뉴를 클릭한 후 [CLOVA Studio 바로가기] 버튼을 클릭해 주십시오.  CLOVA Studio에서 스킬 트레이너 메뉴를 클릭해 주십시오.  화면 상단의 [시나리오 수집] 버튼을 클릭해 주십시오.  화면 왼쪽에서 스킬셋과 엔진을 선택하고 유저쿼리를 입력해 주십시오.  스킬셋: 시나리오를 수집할 스킬셋 선택  포함된 스킬: 선택한 스킬셋에 포함된 스킬 목록이 표시됨. 작업 상태가 '작업 완료'인 스킬만 활용 가능  Engine 및 버전: 사용할 언어 모델 선택. 기본 언어 모델이 기본 값으로 선택되어 있음  호출옵션(선택): 유저쿼리 호출 시 고정 값 입력. 호출옵션을 작성하는 방법은 호출옵션 작성을 참조해 주십시오.  유저쿼리: 사용자 쿼리를 입력하는 필드. 스킬셋을 선택해야 해당 필드가 활성화됨  [실행] 버튼을 클릭해 주십시오.  참고  시나리오 수집을 실행한 후에는 스킬셋, Engine, 유저쿼리를 수정할 수 없습니다. 수정이 필요한 경우, [초기화] 버튼을 클릭하여 작업을 새로 수행해 주십시오. [초기화] 버튼은 시나리오 수집이 완료된 이후에 활성화됩니다.    시나리오 데이터 수정  수집한 시나리오의 데이터를 검토하고 수정하는 방법은 다음과 같습니다.    시나리오 수집을 수행해 주십시오.  플래닝 데이터 영역에서 모델의 사고를 검토해 주십시오.  플래닝 데이터 영역의 스텝을 수정하려면 각 필드를 클릭한 후 수정해 주십시오.  플래닝 데이터 영역의 수정하는 방법은 플래닝 데이터를 참조해 주십시오.  스텝 수정이 완료되면 [적용] 버튼을 클릭해 주십시오.  해당 스텝이 재생성됩니다.  스킬 데이터 영역의 데이터를 수정하려면 각 필드를 클릭한 후 수정해 주십시오.  스킬 데이터 영역을 수정하는 방법은 싱글 스킬 데이터를 참조해 주십시오.  수정이 완료되면 [적용] 버튼을 클릭해 주십시오.  해당 스킬 데이터가 재실행됩니다.  플래닝 데이터 영역에서 최종 답변 필드가 표시되면 화면 오른쪽 하단에 있는 [작업 완료] 버튼을 클릭해 주십시오.  해당 작업을 임시 저장하고 나중에 불러오려면 [임시저장] 버튼을 클릭해 주십시오.  참고  스킬 데이터 영역에서 불필요한 데이터가 생성된 경우, 스킬 데이터 영역의 [Step 끝내기] 버튼을 클릭해 주십시오. 해당 스텝과 최종 답변 스텝 사이에 생성된 모든 스텝이 삭제됩니다. [Step 끝내기] 버튼은 최종 답변 스텝까지 모두 생성된 경우에 활성화됩니다.    플래닝 데이터  플래닝 데이터 영역에는 진행 중인 모델 사고 과정의 모든 데이터가 나타납니다. 유저 쿼리 실행 시 활용할 스킬 및 순서 등을 계획하고 수행하는 플래닝 모델의 사고 과정을 확인할 수 있습니다. 플래닝 데이터 영역은 3단계로 구성됩니다.    1단계: 플래닝 단계  어떤 스킬을 사용할 것인지 결정하는 단계입니다.    2.png    항목	설명  생각	유저 쿼리에 대한 모델의 생각  액션	현재 스텝을 진행하기 위해 필요한 액션 종류  액션 입력	액션 호출에 필요한 입력 값. 액션 입력 영역의 쿼리 필드에는 유저 쿼리가 표기되고, 스킬 필드에는 사용할 스킬이 표기됨  두 개 이상의 스킬이 필요한 경우, “강남역 맛집이랑 렌터카 업체 소개해줘“ 대신 “강남역 맛집 알려줘“ + “강남역 렌터카 업체 소개해줘“라고 쿼리가 분리돼서 입력됨  결과	플래닝 단계의 결과가 입력됨  결과 필드 양식: [{“tool_name”: “사용할 스킬 이름1”,“input”:{“query”:“스킬에 해당하는 쿼리”}}, {“tool_name”: “사용할 스킬 이름2”,“input”:{“query”:“스킬에 해당하는 쿼리”}}]  2단계: 스킬 수행 단계  스킬을 호출하는 단계입니다. 스킬 수행 단계에서는 모든 필드를 수정할 수 없습니다.    3단계: 최종 답변 단계  사용자에게 전달할 답변을 결정하는 단계입니다. [작업 완료] 버튼을 클릭하여 모든 내용을 저장하고 시나리오 생성 작업을 끝낼 수 있습니다.    싱글 스킬 데이터  스킬 데이터 영역에는 호출할 스킬 데이터가 나타납니다. 플래닝 데이터 영역에서 유저 쿼리에 어떤 스킬을 사용할 것인지 계획했다면, 스킬 데이터 영역에서는 호출할 스킬에 대한 상세한 데이터가 나타나고 직접 수정할 수 있습니다. 스킬 데이터 영역은 3단계로 구성됩니다.    1단계: API 조회  싱글 스킬 데이터의 1단계는 API를 조회하는 단계입니다.    3.png    항목	설명  생각	모델의 판단 결과. 스킬의 API 이름이 포함되어야 함  액션	액션의 이름  액션 입력	반드시 'None' 값 표시  관찰	모델의 판단에 따라 사용해야 하는 API 스펙  2단계: API 호출  싱글 스킬 데이터 2단계인 API를 호출하는 단계입니다. 스킬 데이터 2단계인 API를 호출하는 단계입니다. 각 필드를 클릭하여 값을 수정할 수 있습니다. 필드의 값을 수정하면 [적용] 버튼이 활성화됩니다. [적용] 버튼을 클릭하면 스킬 데이터가 재실행되고 수정된 결과가 플래닝 데이터 영역에도 적용됩니다.    API 호출 단계에서 GET 메소드를 사용하는 스킬 데이터의 화면은 다음과 같습니다.    4.png    항목	설명  생각	API를 호출하기 위한 사고 과정. 구체적인 파라미터 및 쿼리 입력 시 성능 향상 가능  <작성 예시> 'API 문서를 확인해보니 필수 파라미터는 {필수 파라미터명}이다. {쿼리}를 하기 위해서는 {사용 파라미터1}의 값으로 {파라미터1 값}을 {사용 파라미터2}의 값으로 {파라미터2 값}을 사용하면 되겠다'.  액션	‘requests_get' 적용  액션입력	API 호출에 필요한 주소 및 파라미터 입력  관찰	모델의 판단 결과  POST 메소드 사용하는 경우에는 액션입력 영역에 URL과 파라미터를 입력할 수 있는 필드가 활성화됩니다.  API 호출 단계에서 POST 메소드를 사용하는 스킬 데이터의 화면은 다음과 같습니다.    5.png    항목	설명  생각	API를 호출하기 위한 사고 과정. 구체적인 파라미터 및 쿼리 입력 시 성능 향상 가능  <작성 예시> 'API 문서를 확인해보니 필수 파라미터는 {필수 파라미터명}이다. {쿼리}를 하기 위해서는 {사용 파라미터1}의 값으로 {파라미터1 값}을 {사용 파라미터2}의 값으로 {파라미터2 값}을 사용하면 되겠다'.  액션	‘requests_post' 적용  액션입력	API 호출 주소와 파라미터 적용  [필드 추가] 버튼: 새로운 파라미터 필드 추가  i-clovastudio_reset : 파라미터 필드 삭제  관찰	모델의 판단 결과  참고  새로운 파라미터 생성 시 기존에 생성된 파라미터의 Key와 중복되지 않도록 설정해 주십시오. 중복된 Key가 있을 경우에는 API가 호출되지 않을 수 있습니다.    3단계: 스킬 답변  API를 조회하고 호출하는 단계를 완료한 후 사용자에게 전달할 답변을 결정하는 단계입니다.    시나리오 구성 방식  사용하는 스킬의 개수에 따라 싱글 스킬 시나리오와 멀티 스킬 시나리오로 구성할 수 있습니다.    싱글 스킬 시나리오  싱글 스킬 시나리오는 스킬을 1개만 활용하는 시나리오입니다. 시나리오 진행 순서는 다음과 같습니다.    시나리오 진행 순서    유저 쿼리: 부산 관광지 추천해줘  플래닝 데이터의 Step 1: 사용할 스킬을 계획하는 단계  플래닝 데이터의 Step 2: 스킬을 수행하는 단계  싱글 스킬 데이터의 Step 2-1: 관광지 정보 API 조회  싱글 스킬 데이터의 Step 2-2: 관광지 정보의 API 파라미터 확인, 키워드 값에 '부산' 사용하여 호출  스킬 데이터의 Step 2-3: 부산 관광지에 대한 결과 출력  플래닝 데이터의 Step 3: 사용자에게 최종 답변하는 단계  시나리오 설명    '부산 관광지 추천해줘'라는 유저 쿼리에 대해 모델은 어떤 스킬을 사용할 것인지 판단합니다. 관광지 조회 API를 호출해야겠다고 판단하면 해당 API를 조회하고 필요한 파라미터를 확인한 후 필요한 정보를 호출합니다. 마지막으로 사용자에게 어떤 형태로 보여줄 지 결정한 후 답변을 제시합니다.    멀티 스킬 시나리오  멀티 스킬 시나리오는 스킬 2개를 활용하는 시나리오입니다. 시나리오 진행 순서는 다음과 같습니다.    시나리오 진행 순서    유저 쿼리: 강남역 맛집과 렌터카 업체 소개해줘  플래닝 데이터의 Step 1: 사용할 스킬을 계획하는 단계  플래닝 데이터의 Step 2: 스킬을 수행하는 단계  싱글 스킬 데이터의 Step 2-1: 강남역 맛집 API 조회  싱글 스킬 데이터의 Step 2-2: 강남역 맛집 API 호출  스킬 데이터의 Step 2-3: 강남역 맛집에 대한 결과 출력  플래닝 데이터의 Step 3: 스킬을 수행하는 단계  싱글 스킬 데이터의 Step 3-1: 렌터카 업체 정보 API 조회  싱글 스킬 데이터의 Step 3-2: 렌터카 업체 정보 API 호출  스킬 데이터의 Step 3-3: 렌터카 업체에 대한 결과 출력  플래닝 데이터의 Step 4: 사용자에게 최종 답변하는 단계  시나리오 설명    '강남역 맛집과 렌터카 업체 소개해줘' 라는 유저 쿼리에 대해 모델은 어떤 스킬을 사용할 것인지 판단합니다. 맛집 조회와 렌터카 업체 조회가 각각 필요하므로 두 개의 API를 호출해야 겠다고 판단합니다. 두 번째 스텝에서는 강남역 주변의 인기 있는 식당을 조회하는 API를 조회하고 호출하고, 세 번째 스텝에서는 렌터카 업체를 조회하는 API를 조회하고 호출합니다. 마지막으로 사용자에게 여러 정보를 어떤 형태로 보여줄 지 결정한 후 답변을 제시합니다.    참고  스킬 수행 단계의 액션 필드에는 호출할 스킬의 'Name for model' 값을 포함해야 합니다.  스킬 수행 단계의 액션 입력 필드 값을 수정할 경우, Step 1의 결과 필드에서 입력된 쿼리를 포함해야 합니다.  시나리오 불러오기  이전에 작업하던 시나리오를 불러와서 검토하고 수정할 수 있습니다.  시나리오를 불러오는 방법은 다음과 같습니다.    네이버 클라우드 플랫폼 콘솔에서 Services > AI Services > CLOVA Studio 메뉴를 차례대로 클릭해 주십시오.  My Product 메뉴를 클릭한 후 [CLOVA Studio 바로가기] 버튼을 클릭해 주십시오.  CLOVA Studio에서 스킬 트레이너 메뉴를 클릭해 주십시오.  화면 상단의 [시나리오 수집] 버튼을 클릭해 주십시오.  시나리오 수집 화면의 왼쪽 하단에 있는 [불러오기] 버튼을 클릭해 주십시오.  작업 중인 시나리오 목록이 나타납니다.  기존 쿼리 목록을 확인해 주십시오.  불러오기 창 우측 상단에서 유저 쿼리를 검색할 수 있습니다.  모델 결과를 확인하려면 데이터 항목의 [보기] 버튼을 클릭해 주십시오.  유저 쿼리를 삭제하려면 목록 우측의 더보기i_clovastudio-more > [삭제] 버튼을 클릭해 주십시오.  시나리오 작업 화면에서 [작업 완료] 버튼을 클릭한 경우에만 작업 상태가 '작업 완료'로 표시됩니다.  시나리오 수집 화면에서 기존 쿼리에 대한 작업을 수정하려면 [열기] 버튼을 클릭해 주십시오.  시나리오 수집 화면이 나타나며 기존 쿼리에 대한 작업을 이어나갈 수 있습니다.  호출옵션 작성  호출옵션을 통해 API spec의 operation ID에 대응하여 헤더 및 본문 매개변수의 특정값을 고정하는 설정을 할 수 있습니다. 호출옵션을 작성하는 방법은 다음과 같습니다.    시나리오 수집 과정 중 화면 왼쪽에서 호출옵션의 [설정 아이콘] 을 클릭해 주십시오.  호출 옵션 설정 창이 나타납니다.  호출값 입력 영역에 호출옵션을 JSON 타입으로 입력해 주십시오. 상단의 예시 포맷을 참고하여 작성해 주십시오.  호출값 입력에 대한 자세한 내용은 상세 호출옵션를 참조해 주십시오.  호출값 입력이 완료되면 [적용] 버튼을 클릭해 주십시오.  적용 실패 시 오류 메시지가 나타납니다. 호출값을 다시 수정해 주십시오.  참고  호출옵션 적용하여 시나리오 수집을 진행한 뒤, [초기화] 버튼을 클릭하더라도 호출옵션은 그대로 유지됩니다. 단, 다음의 경우에 호출옵션이 초기화 됩니다.    시나리오 수집 화면에서 벗어난 후에 스킬셋 목록 또는 스킬 목록에서 [시나리오 수집] 버튼을 클릭하여 다시 시작한 경우  시나리오 수집 화면에서 스킬셋을 변경하는 경우  이러한 경우 시나리오 [불러오기] 를 클릭하여 이전에 사용한 호출옵션을 다시 불러올 수 있습니다.    상세 호출옵션  호출옵션에 작성 가능한 필드에 관한 정보입니다.    필드	데이터 타입	설명  baseOperation	object	해당 스킬셋 내에 등록된 모든 API에 대해 호출옵션이 적용됨  baseOperation.header	map<string, string>	모든 API의 HTTP Header에 추가 및 고정 시키고자 하는 값  baseOperation.query	map<string, string>	모든 API의 HTTP Query Parameter에 추가 및 고정 시키고자 하는 값  baseOperation.requestBody	map<string, object>	모든 API의 HTTP Request Body에 추가 및 고정 시키고자 하는 값  operations	list	해당 스킬셋 내에 등록된 API 중 지정된 Operation ID에 한하여 호출옵션이 적용됨  operations[].operationId	string	호출옵션을 적용하고자 하는 Operation ID를 지정  operations[].header	map<string, string>	지정된 Operation ID에 매핑되는 API의 HTTP Header에 추가 및 고정 시키고자 하는 값  operations[].query	map<string, string>	지정된 Operation ID에 매핑되는 API의 HTTP Query Parameter에 추가 및 고정 시키고자 하는 값  operations[].requestBody	map<string, object>	지정된 Operation ID에 매핑되는 API의 HTTP Request Body에 추가 및 고정 시키고자 하는 값  호출값 작성 예시  스킬셋 내에 등록된 모든 API에 대해 일괄적으로 호출옵션을 적용하고자 할 때 baseOperation 필드를 활용할 수 있습니다. 작성 예시는 다음과 같습니다.    {    "baseOperation": {      "query": {        "serviceKey": "value"      }    }  }  YAML  특정 Operation ID에 한하여 호출옵션을 적용하고자 할 때 operations 필드를 활용할 수 있습니다. 작성 예시는 다음과 같습니다.    {    "operations": [      {        "operationId": "filterBakeryProducts",        "query": {          "serviceKey": "value"        }      }    ]  }  YAML  baseOperation 필드와 operations 필드를 함께 적용하는 것도 가능합니다. 작성 예시는 다음과 같습니다.    {    "baseOperation": {      "header": {        "baseHeader": "baseHeaderValue"      },      "query": {        "baseQuery": "baseQueryValue"      },      "requestBody": {        "baseBody": "baseBodyValue"      }    },    "operations": [      {        "operationId": "testOperationId1",        "header": {          "header1": "headerValue1"        },        "query": {          "query1": "queryValue1"        },        "requestBody": {          "body1": "bodyValue1"        }      },      {        "operationId": "testOperationId2",        "header": {          "header2": "headerValue2"        },        "query": {          "query2": "queryValue2"        },        "requestBody": {          "body2": "bodyValue2"        }      }    ]  }  YAML  참고  호출 옵션은 operations 필드, baseOperation 필드, 유저 쿼리 순으로 적용됩니다."
},
            {
        "category": "스킬셋 버전 관리",
        "context": "스킬셋을 튜닝하거나 정보를 변경하면 해당 내용을 반영한 버전이 생성됩니다. 버전 관리 페이지에서 버전 내용을 확인하고 테스트 앱을 생성할 수 있습니다.    스킬셋 버전 생성 및 갱신  스킬셋 버전이 생성되거나 갱신되는 기준은 다음과 같습니다.    필수 정보를 모두 입력하고 새로운 스킬이 저장되면 해당 스킬을 포함한 스킬셋의 'v1'이 생성됩니다.  스킬셋 내 다른 스킬을 신규로 저장하면 버전이 갱신됩니다.  스킬셋 내 등록된 스킬을 삭제하면 버전이 갱신됩니다.  기존 스킬을 편집하고 저장하면 버전이 갱신됩니다.  시나리오 수집 후 스킬셋 학습이 완료되면 버전이 갱신됩니다.  참고  학습 중일 경우 해당 스킬셋 내 신규 스킬 생성 및 수정이 불가하며 임시 저장만 가능합니다. 학습 완료 후 다시 저장해 주십시오.  버전 관리 화면  clovastudio-skillsetversion_screen_ko.png    영역	설명  ① 정렬 및 검색	  정렬 방식: 최신순, 오래된순, 이름순 정렬 가능  검색: 목록 검색 가능  ② 대표 스킬셋	서비스앱 신청 시 해당 버전이 대표 스킬셋으로 노출됨  스킬셋: 스킬셋 이름  버전: 해당 스킬셋의 버전  메모: 스킬 정보를 변경,삭제하거나 학습 시작 시 작성한 메모  학습 상태: 학습 관련된 버전에 노출되는 학습 진행 상태  생성 일시: 버전이 생성된 일자 및 시간  ③ 버전 이력	스킬셋 버전 이력이 노출되는 영역. 스킬셋당 최대 50개의 버전이 저장되며, 버전이 50개를 초과할 경우 오래된 버전부터 자동으로 삭제됨  ④ 기능 버튼	  [자세히] 버튼: 해당 버전의 스킬 및 학습 정보를 확인할 수 있는 버튼  [테스트앱] 버튼: 테스트 앱 생성 참조  테스트 앱 생성  테스트 앱을 생성하는 방법은 다음과 같습니다.    네이버 클라우드 플랫폼 콘솔에서 Services > AI Services > CLOVA Studio 메뉴를 차례대로 클릭해 주십시오.  My Product 메뉴를 클릭한 후 [CLOVA Studio 바로가기] 버튼을 클릭해 주십시오.  CLOVA Studio에서 우측 상단에 있는 사용자 이름을 클릭한 후 내 작업 메뉴를 클릭해 주십시오.  [스킬 트레이너] 탭을 클릭해 주십시오.  [버전관리] 탭을 클릭해 주십시오.  테스트 앱을 생성할 버전에 [생성] 버튼을 클릭해 주십시오.  테스트 앱 이름을 입력한 후 [생성] 버튼을 클릭해 주십시오.  테스트 앱이 생성되고 테스트 앱 팝업 창이 나타납니다.  clovastuido-skillset-tuning_testapp_ko  테스트 앱의 API 정보를 확인할 수 있습니다. (API에 관한 자세한 내용은 CLOVA Studio API 가이드 참고)  코드 타입은 curl과 python이 제공됩니다.  [복사] 버튼을 클릭하여 API 정보를 클립보드에 복사할 수 있습니다.  [재발급] 버튼을 클릭하여 API Gateway Key를 재발급할 수 있습니다.  참고  테스트 앱을 v1으로 생성하면 기본 모델로 API를 호출할 수 있습니다.  테스트 앱을 생성하여 사용에 문제가 없는지 확인한 후 서비스 앱을 신청할 수 있습니다. 서비스 앱을 신청하려면 서비스 앱 신청 양식을 작성해 주십시오. 서비스 앱 신청에 관한 자세한 내용은 서비스 앱 신청을 참조해 주십시오.  앱 신청 현황을 확인하려면 앱 신청 현황 확인을 참조해 주십시오."
}
           ]
